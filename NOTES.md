# digraphia — design notes

**The research, the rationale, and the dead ends behind
[`README.md`](./README.md).** This is the long-form argument: why the problem is
real, why the link cannot be computed, and why the design landed where it did.
Read the README first if you just want to use the library.
...
```
hackathon-ethbg26/
├── README.md                      library docs — start there
├── NOTES.md                       this document — the research and rationale
...
│           ├── readme.test.ts      4 tests — asserts the README examples
...
37 tests, all passing. The security-relevant ones:
...
`readme.test.ts` asserts that every code example in the README produces exactly
the output it claims, so the docs cannot drift from the implementation.
...
| | |
|---|---|
| ✅ | Core library, 37 tests, live-verified read path |
